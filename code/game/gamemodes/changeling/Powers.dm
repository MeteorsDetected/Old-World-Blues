...
/mob/proc/changeling_darksight()
	...
	if(istype(src,/mob/living/carbon))
		...
		if(C.seedarkness)
			C.darkness_view = 7
			C.see_invisible = SEE_INVISIBLE_OBSERVER_NOLIGHTING
			C << SPAN_NOTE("We allow the shadows to return.")
		else
			C.darkness_view=2
			C << SPAN_NOTE("We no longer need light to see.")
...
/datum/power/changeling/augmented_eyesight
	...
	genomecost = 2
	verbpath = /mob/living/proc/changeling_augmented_eyesight


/mob/living/proc/changeling_augmented_eyesight()
	set category = "Changeling"
	...
	if(C.see_in_dark != 8 )
		C << SPAN_NOTE("We feel a minute twitch in our eyes, and darkness creeps away.")
		C.sight |= SEE_MOBS
...
//		C.dna.species.invis_sight = SEE_INVISIBLE_MINIMUM
		...
		C << SPAN_NOTE("Our vision dulls. Shadows gather.")
		C.sight &= ~SEE_MOBS
//		C.permanent_sight_flags &= ~SEE_MOBS
		C.see_in_dark = 2

...
/datum/game_mode/malfunction/verb/machine_overload(obj/machinery/M in machines)
	...
	if(N && istype(N)) // /obj/machinery/power first, these create bigger explosions due to direct powernet connection
		if(!istype(N, /obj/machinery/power/apc) && !istype(N, /obj/machinery/power/smes/buildable) && (!N.powernet || !N.powernet.avail)) // Directly connected machine which is not an APC or SMES. Either it has no powernet connection or it's powernet does not have enough power to overload
			user << SPAN_NOTE("ERROR: Low network voltage. Unable to overload. Increase network power level and try again.")
			return
			...
			if(A.cell && A.cell.charge)
				...
				user << SPAN_NOTE("ERROR: APC Malfunction - Cell depleted or removed. Unable to overload.")
				return
			...
			if(S.charge && S.RCon)
				...
				if(!S.charge)
					user << SPAN_NOTE("ERROR: SMES Depleted. Unable to overload. Please charge SMES unit and try again.")
				else
					user << SPAN_NOTE("ERROR: SMES RCon error - Unable to reach destination. Please verify wire connection.")
				return
		...
		if(!M.use_power) // Not using power at all
			user << SPAN_NOTE("ERROR: No power grid connection. Unable to overload.")
			return
		if(M.inoperable()) // Not functional
			user << SPAN_NOTE("ERROR: Unknown error. Machine is probably damaged or power supply is nonfunctional.")
			return
	else // Not a machine at all (what the hell is this doing in Machines list anyway??)
		user << SPAN_NOTE("ERROR: Unable to overload - target is not a machine.")
		return
	...
	M.visible_message(SPAN_NOTE("BZZZZZZZT"))
	spawn(50)

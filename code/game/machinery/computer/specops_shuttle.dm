...
//		announcer.autosay(message, "A.L.I.C.E.", "Response Team")
	...
	for(var/turf/T in get_area_turfs(end_location) )
		var/mob/M = locate(/mob) in T
		M << SPAN_NOTE("You have arrived to [station_name]. Commence operation!")
...
/obj/machinery/computer/specops_shuttle/emag_act(var/remaining_charges, var/mob/user)
	user << SPAN_NOTE("The electronic systems in this console are far too advanced for your primitive hacking peripherals.")
...
/obj/machinery/computer/specops_shuttle/Topic(href, href_list)
	...
	if (href_list["sendtodock"])
		...
		if (!specops_can_move())
			usr << SPAN_NOTE("Central Command will not allow the Special Operations shuttle to return yet.")
			if(world.timeofday <= specops_shuttle_timereset)
				if (((world.timeofday - specops_shuttle_timereset)/10) > 60)
					usr << SPAN_NOTE("[-((world.timeofday - specops_shuttle_timereset)/10)/60] minutes remain!")
				usr << SPAN_NOTE("[-(world.timeofday - specops_shuttle_timereset)/10] seconds remain!")
			return

		usr << SPAN_NOTE("The Special Operations shuttle will arrive at Central Command in [(SPECOPS_MOVETIME/10)] seconds.")
		...
		usr << SPAN_NOTE("The Special Operations shuttle will arrive on [station_name] in [(SPECOPS_MOVETIME/10)] seconds.")

	...
	for(var/mob/living/simple_animal/bee/B in T)
		if(B.feral < 0)
			...
			user.visible_message(
				SPAN_NOTE("[user] nets some bees."),
				SPAN_NOTE("You net up some of the becalmed bees.")
			)
		else
			user.visible_message(
				"\red [user] swings at some bees, they don't seem to like it.",
				"\red You swing at some bees, they don't seem to like it."
			)
			B.feral = 5
			B.target_mob = user
...
/obj/item/weapon/reagent_containers/food/snacks/honeycomb
	...
	bitesize = 2
	preloaded = list("honey" = 10, "nutriment" = 0.5, "sugar" = 2)

	...
	Topic(href, href_list)
		...
		if (src.authenticated)
				...
				switch(href_list["field"])
					if("fingerprint")
						if (istype(src.active1, /datum/data/record))
							...
							if ((!( t1 ) || !( src.authenticated ) || usr.incapacitated() || (!interactable() && (!issilicon(usr))) || src.active1 != a1))
								return
					...
					if("mi_dis")
						if (istype(src.active2, /datum/data/record))
							...
							if ((!( t1 ) || !( src.authenticated ) || usr.incapacitated() || (!interactable() && (!issilicon(usr))) || src.active2 != a2))
								return
			...
			if (href_list["search"])
				...
				if ((!( t1 ) || usr.incapacitated() || !( src.authenticated ) || ((!interactable()) && (!issilicon(usr)))))
					return
				src.active1 = null
				src.active2 = null
				t1 = lowertext(t1)

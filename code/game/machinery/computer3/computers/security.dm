...
/*			if ("Search Fingerprints")
				...
				if ((!( t1 ) || usr.incapacitated() || !authenticated || (!interactable()) && (!issilicon(usr))))
					return
				active1 = null
				active2 = null
				t1 = lowertext(t1)
				...
				switch(href_list["field"])
					if("name")
						if (istype(active1, /datum/data/record))
							...
							if ((!( t1 ) || !length(trim(t1)) || !( authenticated ) || usr.incapacitated() || (!interactable() && (!issilicon(usr)))) || active1 != a1)
								return
							active1.fields["name"] = t1
					if("id")
						if (istype(active2, /datum/data/record))
							var/t1 = sanitize(input("Please input id:", "Secure. records", active1.fields["id"], null)  as text)
							if ((!( t1 ) || !( authenticated ) || usr.incapacitated() || (!interactable() && (!issilicon(usr))) || active1 != a1))
								return

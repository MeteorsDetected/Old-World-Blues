...
/obj/machinery/space_heater/examine(mob/user)
	...
	user << SPAN_NOTE("The heater is [on ? "on" : "off"] and the hatch is [panel_open ? "open" : "closed"].")
	if(panel_open)
		user << SPAN_NOTE("The power cell is [cell ? "installed" : "missing"].")
	else
		user << SPAN_NOTE("The charge meter reads [cell ? round(cell.percent(),1) : 0]%")
	return
...
/obj/machinery/space_heater/attackby(obj/item/I, mob/user)
	if(istype(I, /obj/item/weapon/cell))
		if(panel_open)
			if(cell)
				user << SPAN_NOTE("There is already a power cell inside.")
				return
			else
				// insert cell
			...
			user << SPAN_NOTE("The hatch must be open to insert a power cell.")
			return

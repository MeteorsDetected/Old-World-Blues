...
/obj/item/storage/secure/examine(mob/user, return_dist=1)
	.=..()
	if(.<=1)
	...
	if(locked)
		...
		if (istype(W, /obj/item/weapon/screwdriver))
			if (do_after(user, 20, src))
				src.open =! src.open
				user.show_message(SPAN_NOTE("You [src.open ? "open" : "close"] the service panel."))
			return
		if ((istype(W, /obj/item/device/multitool)) && (src.open == 1) && (!src.l_hacking))
			user.show_message(SPAN_NOTE("Now attempting to reset internal memory, please hold."), 1)
			src.l_hacking = 1
			if (do_after(usr, 100, src))
				if (prob(40))
					src.l_setshort = 1
					src.l_set = 0
					user.show_message(SPAN_NOTE("Internal memory reset. Please give it a few seconds to reinitialize."), 1)
					sleep(80)
					src.l_setshort = 0
					src.l_hacking = 0
				else

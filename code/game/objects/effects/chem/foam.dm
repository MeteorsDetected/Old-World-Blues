...
/obj/structure/foamedmetal/attack_hand(var/mob/user)
	...
	if ((HULK in user.mutations) || (prob(75 - metal * 25)))
		user.visible_message(
			SPAN_WARN("[user] smashes through the foamed metal."),
			SPAN_NOTE("You smash through the metal foam wall.")
		)
		qdel(src)
		return
	*/
...
/obj/structure/foamedmetal/attackby(var/obj/item/I, var/mob/user)
	...
	if(prob(I.force * 20 - metal * 25))
		user.visible_message(
			SPAN_WARN("[user] smashes through the foamed metal."),
			SPAN_NOTE("You smash through the foamed metal with \the [I].")
		)
		qdel(src)
	else
		user << SPAN_NOTE("You hit the metal foam to no effect.")

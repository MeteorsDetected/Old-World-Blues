...
/obj/structure/loot_pile
	...
	var/loot_depletion = TRUE		// If true, loot piles can be 'depleted' after a certain number of searches by different players, where no more loot can be obtained.
	var/loot_left = 5				// When this reaches zero, and loot_depleted is true, you can't obtain anymore loot.
	var/delete_on_depletion = TRUE	// If true, and if the loot gets depleted as above, the pile is deleted.

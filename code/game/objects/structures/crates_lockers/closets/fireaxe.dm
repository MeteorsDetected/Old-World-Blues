...
/obj/structure/closet/fireaxecabinet
	...
	attackby(var/obj/item/O as obj, var/mob/user as mob)  //Marker -Agouri
		...
		if (istype(O, /obj/item/weapon/material/twohanded/fireaxe) && src.localopened)
			if(!fireaxe)
				...
				src.contents += O
				user << SPAN_NOTE("You place the fire axe back in the [src.name].")
				update_icon()
			else
				if(src.smashed)
					return
	...
	attack_hand(mob/user as mob)
		...
		if(localopened)
			if(fireaxe)
				...
				fireaxe = null
				user << SPAN_NOTE("You take the fire axe from the [name].")
	...
	attack_tk(mob/user as mob)
		if(localopened && fireaxe)
			...
			user << SPAN_NOTE("You telekinetically remove the fire axe.")
			fireaxe = null
			update_icon()
			return
		attack_hand(user)
	...
	verb/toggle_openness() //nice name, huh? HUH?! -Erro //YEAH -Agouri
		...
		if (isrobot(usr) || src.locked || src.smashed)
			if(src.locked)
				...
				usr << SPAN_NOTE("The protective glass is broken!")
			return
	...
	verb/remove_fire_axe()
		...
		if (localopened)
			if(fireaxe)
				...
				fireaxe = null
				usr << SPAN_NOTE("You take the Fire axe from the [name].")
			else
				usr << SPAN_NOTE("The [src.name] is empty.")
		else
			usr << SPAN_NOTE("The [src.name] is closed.")
		update_icon()
	...
	attack_ai(mob/user as mob)
		if(src.smashed)
			...
			if(locked)
				...
				user << SPAN_NOTE("Cabinet unlocked.")
			return

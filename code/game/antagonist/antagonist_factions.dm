...
/mob/living/proc/convert_to_faction(var/datum/mind/player, var/datum/antagonist/faction)
	...
	if(player_is_antag(player) || !faction.can_become_antag(player))
		to_chat(player, "<span class='danger'>The [src] is trying to force you to join the [faction.faction_descriptor]! With no chance of success, actually.</span>" )
		...
		if(choice == "No!" || choice == "Yes!(no)")
			to_chat(player, "<span class='danger'>You had literally no choice!</span>" )
	...
	if(choice == "Yes!" && faction.add_antagonist_mind(player, 0, faction.faction_role_text, faction.faction_welcome))
		to_chat(src,  SPAN_NOTE("\The [player.current] joins the [faction.faction_descriptor]!") )
		return
	if(choice == "No!")

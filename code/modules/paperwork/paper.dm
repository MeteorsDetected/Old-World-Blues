...
/obj/item/weapon/paper/verb/rename()
	...
	if(src.Adjacent(usr) && usr.Adjacent(src) && !usr.incapacitated() && n_name)
		name = n_name
		if(n_name != "paper")
			desc = "This is a paper titled '" + name + "'."
...
/obj/item/weapon/paper/attack(mob/living/carbon/M as mob, mob/living/carbon/user as mob)
	if(user.zone_sel.selecting == O_EYES)
		user.visible_message(
			SPAN_NOTE("[user] holds up a paper and shows it to [M]."),
		...
		if(ishuman(M))
			...
			if(H == user)
				...
				if (do_mob(user, H, 10))//user needs to keep their active hand, H does not.
					user.visible_message(
						SPAN_NOTE("[user] wipes [H]'s lipstick off with \the [src]."),
...
/obj/item/weapon/paper/proc/burnpaper(obj/item/weapon/flame/P, mob/user)
	...
	if(P.lit && !user.restrained())
		...
		user.visible_message(
			"[class][user] holds \the [P] up to \the [src], it looks like \he's trying to burn it!",
		...
		spawn(20)
			if(get_dist(src, user) < 2 && user.get_active_hand() == P && P.lit)
				user.visible_message(
					"[class][user] burns right through \the [src], turning it to ash. It flutters through the air before settling on the floor in a heap.",
				...
				user << SPAN_WARN("You must hold \the [P] steady to burn \the [src].")
...
/obj/item/weapon/paper/Topic(href, href_list)
	...
	if(href_list["write"])
		...
		if(!src.Adjacent(usr) || !usr.Adjacent(src))
			return
...
/obj/item/weapon/paper/attackby(obj/item/weapon/P as obj, mob/user as mob)
	...
	if(istype(P, /obj/item/weapon/paper) || istype(P, /obj/item/weapon/photo))
		...
		if ( istype(RP) && RP.mode == 2 )
			...
			show_content(user, SHOWLINKS)
		return

	else if(istype(P, /obj/item/weapon/stamp))
		if(!user.Adjacent(src) || !src.Adjacent(user))
			return
...
/obj/item/weapon/paper/courtroom
	...
	info = {"
		...
		After the crime has been committed the Detective's job is to gather evidence and try to ascertain not only who did it but what happened.
		He must take special care to catalogue everything and don't leave anything out.
		Write out all the evidence on paper. Make sure you take an appropriate number of fingerprints.
		IF he must ask someone questions he has permission to confront them. If the person refuses he can ask a judicial authority to write a subpoena for questioning.
		If again he fails to respond then that person is to be jailed as insubordinate and obstructing justice. Said person will be released after he cooperates.<BR>
		<BR>
		ONCE the FT has a clear idea as to who the criminal is he is to write an arrest warrant on the piece of paper. IT MUST LIST THE CHARGES.
		The FT is to then go to the judicial authority and explain a small version of his case. If the case is moderately acceptable the authority should sign it.
		Security must then execute said warrant.<BR>
		<BR>
		<B>Pre-Pre-Trial Phase:</B><BR>
		Now a legal representative must be presented to the defendant if said defendant requests one.
		That person and the defendant are then to be given time to meet (in the jail IS ACCEPTABLE).
		The defendant and his lawyer are then to be given a copy of all the evidence that will be presented at trial (rewriting it all on paper is fine).
		THIS IS CALLED THE DISCOVERY PACK. With a few exceptions, THIS IS THE ONLY EVIDENCE BOTH SIDES MAY USE AT TRIAL.
		IF the prosecution will be seeking the death penalty it MUST be stated at this time.
		ALSO if the defense will be seeking not guilty by mental defect it must state this at this time to allow ample time for examination.<BR>
		Now at this time each side is to compile a list of witnesses. By default, the defendant is on both lists regardless of anything else.
		Also the defense and prosecution can compile more evidence beforehand BUT in order for it to be used the evidence MUST also be given to the other side.
		...
		4. <U>Drop the Charges-</U> Not enough evidence is there for a trial so the charges are to be dropped.
		The FT CAN RETRY but the judicial authority must carefully reexamine the new evidence.<BR>
		5. <U>Declare Incompetent-</U> Basically the defendant is insane. Once this is granted a medical official is to examine the patient.
		If he is indeed insane he is to be placed under care of the medical staff until he is deemed competent to stand trial.<BR>
		<BR>
		ALL SIDES MOVE TO A COURTROOM<BR>
		<B>Pre-Trial Hearings:</B><BR>
		A judicial authority and the 2 sides are to meet in the trial room. NO ONE ELSE BESIDES A SECURITY DETAIL IS TO BE PRESENT.
		The defense submits a plea. If the plea is guilty then proceed directly to sentencing phase. Now the sides each present their motions to the judicial authority.
		He rules on them. Each side can debate each motion. Then the judicial authority gets a list of crew members.
		He first gets a chance to look at them all and pick out acceptable and available jurors.
		Those jurors are then called over. Each side can ask a few questions and dismiss jurors they find too biased.
		HOWEVER before dismissal the judicial authority MUST agree to the reasoning.<BR>
		<BR>
		<B>The Trial:</B><BR>
		...
		2. <B>Witness Calling/Evidence Presentation</B>- The prosecution goes first and is able to call the witnesses on his approved list in any order.
		He can recall them if necessary. During the questioning the lawyer may use the evidence in the questions to help prove a point.
		After every witness the other side has a chance to cross-examine.
		After both sides are done questioning a witness the prosecution can present another or recall one (even the EXACT same one again!).
		After prosecution is done the defense can call witnesses. After the initial cases are presented both sides are free to call witnesses on either list.<BR>
		...
		REMEMBER: They mix between some charges being guilty and others not guilty
		(IE if you supposedly killed someone with a gun and you unfortunately picked up a gun without authorization
		then you CAN be found not guilty of murder BUT guilty of possession of illegal weaponry.). Once they have agreed they present their verdict.
		If unable to reach a verdict and feel they will never they call a deadlocked jury and we restart at Pre-Trial phase with an entirely new set of jurors.<BR>
		<BR>
		...
		I. Each side can present more evidence/witnesses in any order.
		There is NO ban on emotional aspects or anything. The prosecution is to submit a suggested penalty.
		After all the sides are done then the judicial authority is to give a sentence.<BR>
		II. The jury stays and does the same thing as I. Their sole job is to determine if the death penalty is applicable.
		If NOT then the judge selects a sentence.<BR>
		<BR>
...
/obj/item/weapon/paper/djstation
	...
	info = {"
		<B>Welcome new owner!</B><BR><BR>
		You have purchased the latest in listening equipment.
		The telecommunication setup we created is the best in listening to common and private radio fequencies.
		Here is a step by step guide to start listening in on those saucy radio channels:<br>
		<ol>
		...
		Simple as that. Now to listen to the private channels, you'll have to configure the intercoms, located on the front desk.
		Here is a list of frequencies for you to listen on.<br>
		<ul>
			<li>145.7 - Common Channel</li>
			...
			<li>134.9 - Mining Channel</li>
			<li>134.7 - Cargo Channel</li>
		</ul>
	"}

/obj/item/weapon/paper/cookies
	name = "message"
	info = "<B>Hi this Is 'Tim' at the bakery. The cookies that you ordered should be delivered by now...\
		A list of ingredients are included... Make sure that you read them carefully!</B>"

/obj/item/weapon/paper/CVIT
	name = "CVIT crew note"
	icon_state = "paper_words"
	info = {"
		Aye aye, guys, today you are hauling something very valuable. This is why you are instructed to bring three safes, two of them decoy.
		Whatever it is you are guarding, it must be very important.
		No jacking around - as soon as you leave planetary space, you are on your own, `cause we are still lacking licence for armed vessel maneuvers in this sector.
		And yeah,  and we`re still playing ball this weekend, Kev, so try and get back in time.
	"}
...
/obj/item/weapon/paper/adminanounce
	...
	icon_state = "paper_words"
	info = {"
		It is reported that merely closing down leisure facilities has not been successful.
		You and your Heads of Staff are to ensure that all crew are working hard, and not wasting time or energy.
		Any crew caught off duty without leave from their Head of Staff are to be warned, and on repeated offence,
		to be brigged until the next transfer shuttle arrives, which will take them to facilities where they can be of more use.
	"}

		...
		if(0)
			...
			if(src.arcanecheckout)
				...
				user.visible_message(
					SPAN_NOTE("\The [user] stares at the blank screen for a few moments, \his expression \
					frozen in fear. When \he finally awakens from it, \he looks a lot older."), 2
				)
						...
						if(!dbcon.IsConnected())
							...
							*/
							...
							if(!query.Execute())
								...
								log_game("[usr.name]/[usr.key] has uploaded the book titled [scanner.cache.name], [length(scanner.cache.dat)] signs", src, 0)

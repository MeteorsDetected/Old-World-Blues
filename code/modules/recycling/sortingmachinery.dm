...
/obj/structure/bigDelivery/examine(mob/user, return_dist=1)
	.=..()
	if(.<=4)
		if(sortTag)
...
/obj/item/smallDelivery/examine(mob/user, return_dist=1)
	.=..()
	if(.<=4)
		if(sortTag)
			user << SPAN_NOTE("It is labeled \"[sortTag]\"")
...
/obj/item/weapon/packageWrap/afterattack(var/obj/target as obj, mob/user as mob, proximity)
	...
	if (istype(target, /obj/item) && !(istype(target, /obj/item/storage) && !istype(target,/obj/item/storage/box)))
		...
		if (src.amount > 3 && !O.opened)
			var/obj/structure/bigDelivery/P = new (get_turf(O.loc))
			P.wrapped = O
			O.welded = 1
			O.forceMove(P)
			src.amount -= 3

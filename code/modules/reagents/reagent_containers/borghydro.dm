/obj/item/weapon/reagent_containers/borghypo
	...
	var/list/reagent_ids = list("tricordrazine", "inaprovaline", "spaceacillin", "bicaridine", "dermaline", "anti_toxin", "dexalin")
	var/list/reagent_volumes = list()
	var/list/reagent_names = list()

/obj/item/weapon/reagent_containers/borghypo/surgeon
	reagent_ids = list("bicaridine", "inaprovaline", "dexalin", "dermaline", "anti_toxin", "tricordrazine", "tramadol",)
...
/obj/item/weapon/reagent_containers/borghypo/attack(var/mob/living/M, var/mob/user)
	...
	if(M.can_inject(user, 1))
		user << SPAN_NOTE("You inject [M] with the injector.")
		...
		if(M.reagents)
			...
			user << SPAN_NOTE("[t] units injected. [reagent_volumes[reagent_ids[mode]]] units remaining.")
	return
...
/obj/item/weapon/reagent_containers/borghypo/Topic(var/href, var/list/href_list)
	if(href_list["reagent"])
		...
		if(t)
			...
			usr << SPAN_NOTE("Synthesizer is now producing '[R.name]'.")

/obj/item/weapon/reagent_containers/borghypo/examine(mob/user)
	.=..()
	if(.<=2)
		var/datum/reagent/R = chemical_reagents_list[reagent_ids[mode]]
		user << SPAN_NOTE("It is currently producing [R.name] and has [reagent_volumes[reagent_ids[mode]]] out of [volume] units left.")
...
/obj/item/weapon/reagent_containers/borghypo/service/afterattack(var/obj/target, var/mob/user, var/proximity)
	...
	if(!reagent_volumes[reagent_ids[mode]])
		user << SPAN_NOTE("[src] is out of this reagent, give it some time to refill.")
		return
	...
	reagent_volumes[reagent_ids[mode]] -= t
	user << SPAN_NOTE("You transfer [t] units of the solution to [target].")
	return

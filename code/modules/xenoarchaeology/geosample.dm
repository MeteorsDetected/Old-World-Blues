...
/datum/geosample/proc/UpdateNearbyArtifactInfo(var/turf/simulated/mineral/container)
	...
	if(container.artifact_find)
		...
		//if(Xenoarch) //Sanity check due to runtimes ~Z
		for(var/turf/simulated/mineral/T in Xenoarch.artifact_spawning_turfs)
			if(T.artifact_find)
				...
				Xenoarch.artifact_spawning_turfs.Remove(T)

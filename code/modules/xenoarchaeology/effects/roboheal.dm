...
/datum/artifact_effect/roboheal/DoEffectTouch(var/mob/user)
	if(user)
		if (isrobot(user))
			...
			R << SPAN_NOTE("Your systems report damaged components mending by themselves!")
	...
	if(holder)
		...
		for (var/mob/living/silicon/robot/M in range(src.effectrange,T))
			if(world.time - last_message > 200)
				M << SPAN_NOTE("SYSTEM ALERT: Beneficial energy field detected!")
...
/datum/artifact_effect/roboheal/DoEffectPulse()
	if(holder)
		...
		for (var/mob/living/silicon/robot/M in range(src.effectrange,T))
			if(world.time - last_message > 200)
				M << SPAN_NOTE("SYSTEM ALERT: Structural damage has been repaired by energy pulse!")
				last_message = world.time

		...
		if(BP_L_HAND, BP_R_HAND)
			...
			if(c_hand && (stun_amount || agony_amount > 10))
				...
				if (affected.robotic >= ORGAN_ROBOT)
					custom_emote(MESSAGE_VISIBLE, "drops what they were holding, their [affected.name] malfunctioning!")
				else
					var/emote_scream = pick("screams in pain and ", "lets out a sharp cry and ", "cries out and ")
					custom_emote(MESSAGE_VISIBLE, "[(species && species.flags & NO_PAIN) ? "" : emote_scream ]drops what they were holding in their [affected.name]!")

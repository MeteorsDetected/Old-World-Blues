	...
	if(stance_damage >= 4 || (stance_damage >= 2 && prob(5)))
		if(!(lying || resting))
			if(species && !(species.flags & NO_PAIN))
				emote("scream")
			custom_emote(MESSAGE_VISIBLE, "collapses!")
...
/mob/living/carbon/human/proc/handle_grasp()
	...
	for (var/obj/item/organ/external/E in organs)
		...
		if(E.is_broken() || E.is_dislocated())
			...
			custom_emote(MESSAGE_VISIBLE, "[(species.flags & NO_PAIN) ? "" : emote_scream ]drops what they were holding in their [E.name]!")
			...
			custom_emote(MESSAGE_VISIBLE, "drops what they were holding, their [E.name] malfunctioning!")

...
/obj/item/device/soulstone/attack(mob/living/carbon/human/M as mob, mob/user as mob)
	...
	admin_attack_log(user, M,
		"Used the [src.name] to capture the soul of [key_name(M)]",
		"Has had their soul captured with [src.name] by [key_name(user)]",
		"used the [src.name] to capture the soul of"
	)
				...
				if ((T.health + T.halloss) > config.health_threshold_crit)
					...
					if(T.client == null)
						...
						if(C.contents.len)
							...
							U << SPAN_NOTE("<b>Capture successful!</b>: \black [T.real_name]'s soul has been ripped from their body and stored within the soul stone.")
			...
			if (T.stat == DEAD)
				...
				if(C.contents.len)
					...
					if(T.name != C.imprinted)
						...
						U << SPAN_NOTE("<b>Capture successful!</b>: \black [T.name]'s has been recaptured and stored within the soul stone.")
		if("CONSTRUCT")

...
/mob/living/simple_animal/corgi/attackby(var/obj/item/O as obj, var/mob/user as mob)  //Marker -Agouri
	if(istype(O, /obj/item/weapon/newspaper))
		if(!stat)
			user.visible_message(SPAN_NOTE("[user] baps [name] on the nose with the rolled up [O]"))
			spawn(0)
...
/mob/living/simple_animal/corgi/regenerate_icons()
	...
	if(facehugger)
		if(istype(src, /mob/living/simple_animal/corgi/puppy))
			overlays += image('icons/mob/corgi_head.dmi',"facehugger_corgipuppy")
		else
			overlays += image('icons/mob/corgi_head.dmi',"facehugger_corgi")

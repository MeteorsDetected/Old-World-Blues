	...
	switch(stance)
			...
			if(target_mob && target_mob in ListTargets(10))
				if(!(SA_attackable(target_mob)))
					...
					if(stance_step in list(1,4,7)) //every 3 ticks
						...
						if(action)
							custom_emote(MESSAGE_VISIBLE, action)
			if(!found_mob)
				stance_step--
		...
		if(HOSTILE_STANCE_ATTACKING)
			if(stance_step >= 20)	//attacks for 20 ticks, then it gets tired and needs to rest
				custom_emote(MESSAGE_VISIBLE, "is worn out and needs to rest." )
...
/mob/living/simple_animal/hostile/bear/FindTarget()
	. = ..()
	if(.)
		custom_emote(MESSAGE_VISIBLE, "stares alertly at [.]")
		stance = HOSTILE_STANCE_ALERT
...
/mob/living/simple_animal/hostile/bear/AttackingTarget()
	if(!Adjacent(target_mob))
		return
	custom_emote(MESSAGE_VISIBLE, pick( list("slashes at [target_mob]", "bites [target_mob]") ) )

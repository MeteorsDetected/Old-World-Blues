...
/N */
ADMIN_VERB_ADD(/client/proc/respawn_character, R_SPAWN|R_REJUVINATE)
...
/client/proc/spawn_special()
	...
	var/list/specials = list("Fax")
	var/select = input("Select item for spawning", "Select item") as null|anything in specials
	if(!select)
		return

...
* Пуллы не должны содержать отладочный код(debug code) и проки связанные с ним.
* Пуллы должны содержать осознанное название, отражающие краткое содержание изменений.
* Пуллы не должны содержать код, который не связан с фукционалом, описанным в пуле.
* Если пулл содержит изменение спрайтов или карты следует использовать соответсвующий спрайт- карто- мержер из папки tools. Также следует добавлять скриншот в описание изменений.
* Пуллы не должны содержать закомментированный код, за исключением TODO'ов и поясняющего текста.
* КОд должен наиболее читабельным для остальных разработчиков и иметь одинаковую стилистику.

### ООП
Dream Maker является объектно-ориентированным языком, код должен быть объектно-ориентированным, чтобы быть более гибким при добавлении к нему контента.
